...
public interface CurrencyRateDao extends CrudRepository<CurrencyRate, Long> {
	...
	@Query(value = "select * from currency_rates join currencies on (currencies.id_currency = currency_rates.id_currency) join rate_sessions on (rate_sessions.id_rate_session = currency_rates.id_rate_session) where currency_code = :currencyCode order by mid desc limit :topCount", nativeQuery=true)
